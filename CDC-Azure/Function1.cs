    ...
    Protocol = BrokerProtocol.Plaintext)] string kafkaEvent)
        {
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(sqlConnectionString))
                {
                    ...
                    var insertLogCmd = new SqlCommand("INSERT INTO KafkaLogs (Offset, Partition, Topic, Timestamp, Value, MessageKey) VALUES (@offset, @partition, @topic, @timestamp, @value, @key)", conn);
